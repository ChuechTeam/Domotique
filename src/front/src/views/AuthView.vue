<template>
  ...
  // 🔐 Simuler vos comptes d'inscription (remplace plus tard avec une requête backend)
  const fakeUsers = [
    { username: 'admin', email: 'admin@ehpad.fr', password: '123456' },
    { username: 'saad', email: 'saad@gmail.com', password: 'azerty' },
    { username: 'nour', email: 'nour@ehpad.fr', password: 'bonjour' }
  ]
  ...
  const handleLogin = () => {
    ...
    if (user) {
      error.value = ''
      router.push('/dashboard') // à adapter
    } else {
      error.value = 'Identifiants incorrects. Vérifiez votre email ou mot de passe.'
    }
  }
